					...
					<div class="row">
						...
						<div class="col-md-8">
							<div class="pull-left h4">
								<h4>#{$slide.id_slide} - {$slide.title}</h4>
								{if $slide.is_shared}
									<div>
								...
								{/if}
								<br/>
								<span>{l s='Du %s au %s' sprintf=[$slide.date_from, $slide.date_to] d='Modules.Imageslider.Admin'}</span><br />
								<br/>
								<span class="text-{if $slide.active_desktop}success{else}danger{/if}">{l s='Actif sur desktop' d='Modules.Imageslider.Admin'} : {if $slide.active_desktop}<i class="icon-check"></i>{else}<i class="icon-times"></i>{/if}</span><br />
								<span class="text-{if $slide.active_mobile}success{else}danger{/if}">{l s='Actif sur mobile' d='Modules.Imageslider.Admin'} : {if $slide.active_mobile}<i class="icon-check"></i>{else}<i class="icon-times"></i>{/if}<br />
							</div>
